...
class calculate_metrics:
    ...
    def process_result(self, results_dir):
        """
        Process evaluation results and save the output to a results summary file.
        
        Args:
        results_dir (str): Directory containing the result text files
        """
        # Prepare the output file path in the same directory
        output_file_path = os.path.join(results_dir, "evaluation_summary.txt")
        ...
        with open(output_file_path, 'w') as output_file:
            ...
            for eval_type, task_name_list in eval_type_dict.items():
                ...
                for task_name in task_name_list:
                    ...
                    for img_items in chunk_lines:
                        ...
                        if len(img_items) != 2:
                            c +=1
                            continue
                        ## if img_items are not length 2 probably there is some over-generation by the language model causing the txt output to go one line over and creating issues when reading it.
                        assert len(img_items) == 2
                        ...
                        for img_item in img_items:
                            ...
                            # print('img_item.split', img_item.split("\t"))
                            if len(img_item.split("\t")) < 4:
                                continue
                            img_name, question, gt_ans, pred_ans = img_item.split("\t")
                ...
                print_and_write(f"Total score: {scores}\n")
                for task_name, score in task_score_dict.items():
                    print_and_write(f"\t{task_name} score: {score}")
                print_and_write("\n")
                print('number of skipped pairs', c)
