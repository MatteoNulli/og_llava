...
def calculate_acc(df):
    ...
    print(f"eBench {args.experiment} accuracy:", ebench_acc)
    return ebench_acc
...
if __name__ == "__main__":
    ...
    for pred in open(os.path.join(args.result_dir, f"{args.experiment}.jsonl")):
        ...
        pred['text'] = pred['text'].strip(' ')
        # print(pred)
        # except:
        # print('pred', pred)
        # print(df['index'][0], type(df['index'][0]))
        # print(pred["question_id"])
        # print((df['index']) == str(pred['question_id']))
        # print(cur_df.loc[df['index'] == pred['question_id'], 'prediction'])
