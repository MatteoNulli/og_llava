...
class IJepaVisionTower(BaseVisionTower):
    ...
    def load_model(self, device_map=None):
        ...
        if os.path.exists(local_path):
            print(f"Loading `{self.vision_tower_name}` from local path: {local_path}")
            ckpt = torch.load(local_path, map_location=torch.device('cpu'))
        else:
            print(f"Downloading `{self.vision_tower_name}` from url: {url}")
        ...
        print(f"Loaded IJepa model: {self.vision_tower_name} with hidden size: {self._hidden_size}, image size: {self._image_size}, patch size: {self._patch_size}\nSetting requires_grad: {self.unfreeze_mm_vision_tower}")
        self.is_loaded = True
