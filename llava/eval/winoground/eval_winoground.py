...
def eval(args):
    ...
    for index, row in tqdm(questions.iterrows(), total=len(questions)):
        ...
        caption_0 = row["caption_0"]
        caption_1 = row["caption_1"]

        captions = {"caption_0": caption_0, "caption_1": caption_1}

        image_0 = row["image_0"]["bytes"]
        image_1 = row["image_1"]["bytes"]

        images = {"image_0": image_0, "image_1": image_1}
        ...
        # qs = "Does the provided image match the caption? Pay close attention to the word order and its compositionality. Answer in the format of \"Yes or No.\"\n"

        text_qs = f"Does this image present (A) {caption_0} or (B) {caption_1}? Note, you must choose one of the two options (A) or (B)."
        # image_qs = "Which image better aligns with the description "

        conv = conv_templates[args.conv_mode].copy()

        conv.append_message(conv.roles[0], "<image>" + text_qs)
        ...
        for k, caption in captions.items():
            ...
            if "qwen" in args.model_path.lower():
                ...
                # image_qs = (
                #     f"Does this caption {caption} align with"
                #     + "Image (A) <image> or "
                #     + "Image (B) <image> ?\n"
                #     + "Answer with one of the two options (A) or (B)."
                # )

                conv.append_message(conv.roles[0], image_qs)
                conv.append_message(conv.roles[1], None)
            ...
            if k == "caption_0":
                match = False
                # match = re.search(r'first(?!.*second)', outputs.lower())
                # match = re.search('first(?!.*second.*first)(?=.*second|$)', outputs.lower())
                ...
                match = False
                # match = re.search(r'second(?!.*first)', outputs.lower())
                # match = re.search('second(?!.*first.*second)(?=.*first|$)', outputs.lower())
                # match = re.search('second(?!.*first.*second)', outputs.lower())
                first_match = re.search("first", outputs.lower())
                second_match = re.search("second", outputs.lower())
                first_index = first_match.start() if first_match else -1
                second_index = second_match.start() if second_match else -1
            ...
            # # print('match found', match_found)
            # if match and k == 'caption_0':
            #     match_found = True
            #     # print('match found', match_found)

            # # print('match', match)
            # if match and k == 'caption_1' and match_found:
            #     # print('match found', match_found)
            #     image_correct_count += 1
            #     match_found = False
            #     image_correct = True
